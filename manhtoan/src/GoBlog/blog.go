...
func rootHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
...
func articleHandler(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	uri := req.RequestURI
...
func main() {
	...
	normal := func(h http.HandleFunc) httprouter.Handle {
		return httpAdapter(h)
	}

	auth := func(h http.HandleFunc) httprouter.Handle {
		return httpAdapter(authMiddleware(h))
	}

	router := httprouter.New()
	router.GET("/", normal(rootHandler))
	router.GET("/article/:name", auth(articleHandler))

	finalRouter := recoverMiddleware(logMiddleware(router))
